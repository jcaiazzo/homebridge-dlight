...
Resource.add({
  ...
  children: [
    ...
    { tag: "event", name: "ShutDown", xref: "core\xA79.13.7" },
    {
      ...
      details: "The Leave event SHOULD be generated by the bridge when it detects that the associated device has left the non-Matter network.\n> [!NOTE]\n> NOTE: The FabricIndex field has the X conformance, indicating it shall NOT be present. This event, in the context of Bridged Device Basic Information cluster, has no usable fields, but the original Basic Information cluster's field definition is kept for completeness."
    },
    {
      tag: "event",
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          details: 'This field shall indicate the period, in milliseconds, that the server will wait before the "pending active" state expires. See the KeepActive Command description for details.\n> [!NOTE]\n> NOTE: TimeoutMs is a timeout for the request, NOT the time the device will be awake for. The server will wait for up to TimeoutMs for the device. If after TimeoutMs the ICD device does NOT check-in, the server will not perform any actions.'
        }
      ]
    }
  ]
