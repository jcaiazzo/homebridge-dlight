    ...
    children: [
      {
        tag: "attribute",
        name: "FeatureMap",
        xref: "cluster\xA75.4.5",
        children: [
          {
            tag: "field",
            name: "PS",
            xref: "cluster\xA75.4.5.1",
            details: "This feature shall indicate that the closure can be set to discrete positions, including at minimum the FullyOpen and FullyClosed positions."
          },
          {
            tag: "field",
            name: "LT",
            xref: "cluster\xA75.4.5.2",
            details: "This feature shall indicate that the closure can be latched and unlatched. When latched, the feature secures an axis, preventing associated actuators from moving components along that axis."
          },
          {
            tag: "field",
            name: "IS",
            xref: "cluster\xA75.4.5.3",
            details: "This feature shall indicate that the closure is capable of changing its position or state instantaneously. As a result, the Speed feature is not applicable, and the Stop command is not usable. In such closures, the OverallCurrentState attribute shall immediately follow the OverallTargetState attribute. The state transition diagram remains applicable, however, transitions involving the Stop state shall be omitted."
          },
          {
            tag: "field",
            name: "SP",
            xref: "cluster\xA75.4.5.4",
            details: "This feature shall indicate that the closure supports configurable speed during motion toward a target position. The feature uses the values in ThreeLevelAutoEnum to set the supported speed levels."
          },
          {
            tag: "field",
            name: "VT",
            xref: "cluster\xA75.4.5.5",
            details: "This feature shall indicate that the closure can be set to a designated Ventilation position (e.g., partially open)."
          },
          {
            tag: "field",
            name: "PD",
            xref: "cluster\xA75.4.5.6",
            details: "This feature shall indicate that the closure can be set to a dedicated Pedestrian position. The Pedestrian position provides a clear walkway through the closure."
          },
          {
            tag: "field",
            name: "CL",
            xref: "cluster\xA75.4.5.7",
            details: "This feature shall indicate the capability to trigger a calibration procedure. The calibration can either be fully automated, or require manual steps not described in this specification."
          },
          {
            tag: "field",
            name: "PT",
            xref: "cluster\xA75.4.5.8",
            details: "This feature shall indicate that the closure is capable of activating a form of protection, such as protection against wind. A protection is manufacturer-specific, and it could be a simple software limitation, or a mechanical system deployed by the closure."
          },
          {
            tag: "field",
            name: "MO",
            xref: "cluster\xA75.4.5.9",
            details: "This feature shall indicate that the closure can be operated manually by a user, such as to open a window."
          }
        ]
      },
      {
      ...
      },
      {
        tag: "attribute",
        name: "MainState",
        xref: "cluster\xA75.4.7.2",
        details: "Indicates the current operational state of the closure associated with the server.\n> [!NOTE]\n> NOTE: The MainState diagram is provided exclusively for informational purposes only and is an exemplary design of the internals of a closure implementation to help illustrate the aspects of function that are considered by the cluster's normative text."
      },
      {
        tag: "attribute",
        name: "CurrentErrorList",
        xref: "cluster\xA75.4.7.3",
      ...
      },
      {
        ...
        xref: "cluster\xA75.4.7.4",
        details: 'Indicates the current Position, Latch and/or Speed states, whichever are applicable according to the feature flags set.\nNull, if the state is unknown. Examples could be, but are not limited to:\n  - The state of Position/Latch is not known yet because the closure is not calibrated.\n  - The product has lost its Position/Latch state after manual motion during a shutdown.\nThe values of the different fields within the structure of this attribute depend on:\n  - The effects of MoveTo commands.\n  - The effects of SetTarget and Step commands in a Closure Dimension Cluster associated with this cluster, as described in Section 5.4.4, "Association Between Closure Control and Closure Dimension Clusters".\n  - The Stop command.'
      },
      {
        tag: "attribute",
        name: "OverallTargetState",
        xref: "cluster\xA75.4.7.5",
        details: "Indicates the TargetPosition, TargetLatch and/or TargetSpeed values, whichever are applicable according to the feature flags set.\nNull, if the state is unknown. For example after a reboot."
      },
      {
        tag: "attribute",
        name: "LatchControlModes",
        xref: "cluster\xA75.4.7.6",
        details: "This attribute shall specify whether the latch mechanism can be latched or unlatched remotely."
      },
      {
        tag: "event",
        name: "OperationalError",
        xref: "cluster\xA75.4.9.1",
        details: "This event shall be generated when a reportable error condition is detected. A closure that generates this event shall also set the MainState attribute to Error, indicating an error condition.\nThis event shall contain the following fields:"
      },
      {
        tag: "event",
        name: "MovementCompleted",
        xref: "cluster\xA75.4.9.2",
        details: "This event, if supported, shall be generated when the overall operation ends, either successfully or otherwise. For example, the event is sent upon the completion of a movement operation in a blind."
      },
      {
        tag: "event",
        name: "EngageStateChanged",
        xref: "cluster\xA75.4.9.3",
        details: "This event, if supported, shall be generated when the MainStateEnum attribute changes state to and from disengaged, indicating if the actuator is Engaged or Disengaged.\nThis event shall contain the following fields:\n  - True, when the actuator is in a Engaged state, actuator movements possible.\n  - False, when the actuator is in a Disengaged state, preventing any actuator movements."
      },
      {
        tag: "event",
        name: "SecureStateChanged",
        xref: "cluster\xA75.4.9.4",
        details: "This event, if supported, shall be generated when the SecureState field in the OverallCurrentState attribute changes. It is used to indicate whether a closure is securing a space against possible unauthorized entry.\nThis event shall contain the following fields:\n  - True, when the closure is in a secure state, e.g. unauthorized/undetectable access is not possible.\n  - False, when the closure is in an insecure state, e.g. unauthorized/undetectable access is possible."
      },
      {
        tag: "command",
        name: "Stop",
        xref: "cluster\xA75.4.8.1",
      ...
      },
      {
        ...
        children: [
          ...
          {
            tag: "field",
            name: "OpenedAtSignature",
            description: 'Closure is in its "Signature position"',
            xref: "cluster\xA75.4.6.1.1",
      ...
      {
        tag: "datatype",
        name: "TargetPositionEnum",
        xref: "cluster\xA75.4.6.2",
        children: [
      ...
      {
        tag: "datatype",
        name: "ClosureErrorEnum",
        xref: "cluster\xA75.4.6.4",
      ...
      {
        tag: "datatype",
        name: "OverallTargetStateStruct",
        xref: "cluster\xA75.4.6.6",
        children: [
          {
            tag: "field",
            name: "Position",
            xref: "cluster\xA75.4.6.6.1",
            details: "This field shall indicate the target position that the closure is moving to. It shall be null if there is no target position."
          },
          {
            tag: "field",
            name: "Latch",
            xref: "cluster\xA75.4.6.6.2",
            details: "This field shall indicate the desired latching state of the closure. It shall be null if there is no desired latching state."
          },
          {
            tag: "field",
            name: "Speed",
            xref: "cluster\xA75.4.6.6.3",
            details: "This field shall indicate the desired speed at which the closure should perform the movement toward the target position. If no speed value has yet been set, the server shall select and set one of the speed values defined in the ThreeLevelAutoEnum."
          }
        ]
      },
      {
        ...
        xref: "cluster\xA75.4.6.7",
        children: [
          {
            tag: "field",
            name: "RemoteLatching",
            description: "Remote latching capability",
            xref: "cluster\xA75.4.6.7.1",
...
//# sourceMappingURL=closure-control.resource.js.map
