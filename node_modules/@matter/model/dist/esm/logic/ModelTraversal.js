...
let memos;
const aspectCache = /* @__PURE__ */ new WeakMap();
class ModelTraversal {
   ...
   */
  findQualified(scope, path, match, options) {
    if (!scope) {
      return;
    }
    ...
    function resolve(scope2, position = 0) {
      ...
      const matched = match(path[position], scope2, isLeaf);
      if (matched) {
        if (isLeaf) {
          return matched;
        }
      ...
      if (position === 0 && scope2.tag === ElementTag.Matter && path[0] === "Matter") {
        return resolve(scope2, position + 1);
      }
    }
