...
class EndpointStore {
   ...
   * commit/dispose here).
   */
  async set(values, sharedTx) {
    const ownTx = sharedTx === void 0;
    let tx;
    if (ownTx) {
      const maybeTx = this.storage.begin();
      tx = import_general.MaybePromise.is(maybeTx) ? await maybeTx : maybeTx;
    } else {
      tx = sharedTx;
    }
    try {
      const txContext = new import_general.StorageContext(tx, this.storage.thisContexts);
