/**
 * @license
 * Copyright 2022-2026 Matter.js Authors
 * SPDX-License-Identifier: Apache-2.0
 */
/*** THIS FILE IS GENERATED, DO NOT EDIT ***/
import { TemperatureControl } from "@matter/types/clusters/temperature-control";
import { ClusterBehavior } from "../../behavior/cluster/ClusterBehavior.js";
import { Identity } from "@matter/general";
/**
 * TemperatureControlBehavior is the base class for objects that support interaction with
 * {@link TemperatureControl.Cluster}.
 *
 * TemperatureControl.Cluster requires you to enable one or more optional features. You can do so using
 * {@link TemperatureControlBehavior.with}.
 */
export declare const TemperatureControlBehaviorConstructor: ClusterBehavior.Type<typeof ClusterBehavior, TemperatureControl, typeof TemperatureControl, new () => {}, Uncapitalize<string>> & {
    readonly id: "temperatureControl";
};
export interface TemperatureControlBehaviorConstructor extends Identity<typeof TemperatureControlBehaviorConstructor> {
}
export declare const TemperatureControlBehavior: TemperatureControlBehaviorConstructor;
export interface TemperatureControlBehavior extends InstanceType<TemperatureControlBehaviorConstructor> {
}
export declare namespace TemperatureControlBehavior {
    interface State extends InstanceType<typeof TemperatureControlBehavior.State> {
    }
}
//# sourceMappingURL=TemperatureControlBehavior.d.ts.map