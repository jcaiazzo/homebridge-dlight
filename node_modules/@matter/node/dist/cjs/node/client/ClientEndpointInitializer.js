...
class ClientEndpointInitializer extends import_EndpointInitializer.EndpointInitializer {
  ...
  createBacking(endpoint, type) {
    if (type.cluster === void 0) {
      const store2 = this.#store.storeForEndpoint(endpoint).createStoreForLocalBehavior(type.id);
  ...
  get structure() {
    if (this.#structure === void 0) {
      ...
      this.#structure = new import_ClientStructure.ClientStructure(
        this.#node,
        (endpoint, behaviorId) => nodeStore.storeForEndpoint(endpoint).createStoreForBehavior(behaviorId)
      );
      this.#structure.eventEmitter = (0, import_ClientEventEmitter.ClientEventEmitter)(this.#node, this.#structure);
    }
