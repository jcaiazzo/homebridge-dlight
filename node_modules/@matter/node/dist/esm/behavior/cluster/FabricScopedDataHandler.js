...
import { Access, Schema } from "@matter/model";
...
async function sanitizeAttributeData(endpoint, type, cluster, supportedAttributes, allowedIndices) {
  ...
  const fabricScopedAttrs = /* @__PURE__ */ new Set();
  ...
  await node.visit(async (endpoint) => {
    await forBehaviors(endpoint, async (type, cluster, elements) => {
      ...
      if (elements.events.size) {
        const clusterSchema = Schema(type);
        const nsEvents = cluster.events;
        if (nsEvents && clusterSchema.tag === "cluster") {
          const fabricScopedEvents = /* @__PURE__ */ new Set();
          ...
          for (const eventName of elements.events) {
            ...
            if (nsEvent && fabricScopedEvents.has(eventName)) {
              fabricRelevantEvents.add(`${cluster.id}-${nsEvent.id}`);
            }
          }
        }
      }
