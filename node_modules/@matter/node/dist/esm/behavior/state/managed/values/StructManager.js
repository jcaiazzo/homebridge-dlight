...
import { Access, ElementTag, FieldValue, Metatype } from "@matter/model";
import { PhantomReferenceError, SchemaImplementationError, Val } from "@matter/protocol";
...
function StructManager(owner, schema) {
  ...
  for (const member of owner.membersOf(schema)) {
    const name2 = member.propertyName;
...
function configureProperty(supervisor, schema) {
  ...
  const isFabricScopedList = schema.effectiveAccess.fabric === Access.Fabric.Scoped && schema.effectiveMetatype === Metatype.array;
  let defaultReader;
