...
class BdxMessenger {
  ...
  static async create(exchangeProvider, messageTimeout = BDX_TRANSFER_IDLE_TIMEOUT) {
    const exchange = await exchangeProvider.initiateExchange({ protocol: import_types.BDX_PROTOCOL_ID });
  ...
  async send(bdxMessage) {
    await this.exchange.send(bdxMessage.kind, import_BdxMessage.BdxMessage.encode(bdxMessage), {
