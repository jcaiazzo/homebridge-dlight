...
class CaseServerMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  async readSigma1() {
    const { payload } = await this.nextMessage({ type: import_types.SecureMessageType.Sigma1 });
  ...
  async readSigma3() {
    const { payload } = await this.nextMessage({ type: import_types.SecureMessageType.Sigma3 });
...
class CaseClientMessenger extends import_SecureChannelMessenger.SecureChannelMessenger {
  async sendSigma1(sigma1, options) {
    return await this.send(sigma1, import_types.SecureMessageType.Sigma1, import_CaseMessages.TlvCaseSigma1, options);
  }
  async readSigma2(abort) {
    const {
      payload,
  ...
  async sendSigma3(sigma3, options) {
    return await this.send(sigma3, import_types.SecureMessageType.Sigma3, import_CaseMessages.TlvCaseSigma3, options);
  }
}
