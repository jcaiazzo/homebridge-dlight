...
class Noc extends import_OperationalBase.OperationalBase {
  ...
  validateFields() {
    ...
    if (icacId === void 0 && rcacId === void 0) {
      throw new import_general.CertificateError("Issuer RCAC or ICAC ID must be defined for an operational certificate.");
    }
    if (isCa) {
      throw new import_general.CertificateError("Node operational certificate must not be a CA.");
    }
  }
  /**
   ...
   */
  asSignedTlv() {
    return import_operational.OperationalCertificate.TlvNoc.encode({ ...this.cert, signature: this.signature.bytes });
  }
  /**
  ...
  async verify(crypto, root, ica) {
    ...
    if (nodeId === void 0 || Array.isArray(nodeId)) {
      throw new import_general.CertificateError(`Invalid nodeId in NoC certificate: ${import_general.Diagnostic.json(nodeId)}`);
    }
    ...
    if ("icacId" in subject) {
      throw new import_general.CertificateError(`Noc certificate must not contain an icacId.`);
    }
    if ("rcacId" in subject) {
      throw new import_general.CertificateError(`Noc certificate must not contain an rcacId.`);
    }
    if ("vvsId" in subject) {
      throw new import_general.CertificateError(`Noc certificate must not contain a vvsId.`);
    }
    ...
    if (this.cert.extensions.basicConstraints.isCa) {
      throw new import_general.CertificateError(`Noc certificate must not have isCa set to true.`);
    }
    if (!this.cert.extensions.keyUsage.digitalSignature) {
      throw new import_general.CertificateError(`Noc certificate must have keyUsage set to digitalSignature.`);
    }
    ...
    if (subjectKeyIdentifier === void 0) {
      throw new import_general.CertificateError(`Noc certificate must have subjectKeyIdentifier set.`);
    }
    if (subjectKeyIdentifier.byteLength !== 20) {
      throw new import_general.CertificateError(`Noc certificate subjectKeyIdentifier must be 160 bit.`);
    }
    if (authorityKeyIdentifier === void 0) {
      throw new import_general.CertificateError(`Noc certificate must have authorityKeyIdentifier set.`);
    }
    if (authorityKeyIdentifier.byteLength !== 20) {
      throw new import_general.CertificateError(`Noc certificate authorityKeyIdentifier must be 160 bit.`);
    }
    let issuer;
    if (import_general.Bytes.areEqual(authorityKeyIdentifier, root.cert.extensions.subjectKeyIdentifier)) {
      issuer = root;
    } else if (ica !== void 0 && import_general.Bytes.areEqual(authorityKeyIdentifier, ica.cert.extensions.subjectKeyIdentifier)) {
      issuer = ica;
    }
    if (issuer === void 0) {
      throw new import_general.CertificateError(
    ...
    await crypto.verifyEcdsa((0, import_general.PublicKey)(issuer.cert.ellipticCurvePublicKey), this.asUnsignedDer(), this.signature);
  }
}
