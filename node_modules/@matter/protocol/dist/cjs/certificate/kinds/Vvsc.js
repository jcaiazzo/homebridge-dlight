...
class Vvsc extends import_OperationalBase.OperationalBase {
   ...
   */
  asSignedTlv() {
    return import_operational.OperationalCertificate.TlvVvsc.encode({ ...this.cert, signature: this.signature.bytes });
  }
  /**
  ...
  async verify(_crypto) {
    ...
    if (vvsId === void 0 || Array.isArray(vvsId)) {
      throw new import_general.CertificateError(`Invalid vvsId in Vsc certificate: ${import_general.Diagnostic.json(vvsId)}`);
    }
    if ("nodeId" in subject) {
      throw new import_general.CertificateError(`Vsc certificate must not contain a nodeId.`);
    }
    if ("fabricId" in subject) {
      throw new import_general.CertificateError(`Vsc certificate must not contain a fabricId.`);
    }
    if ("caseAuthenticatedTags" in subject) {
      throw new import_general.CertificateError(`Vvsc certificate must not contain a caseAuthenticatedTags.`);
    }
  }
}
