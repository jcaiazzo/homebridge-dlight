...
class Advertisement {
    ...
    try {
      ...
      try {
        if (this.#activity) {
          var _stack = [];
          try {
            const _activity = __using(_stack, closing.join("activity"));
            this.#activity.cancel();
            await this.#activity;
          } catch (_) {
        ...
        try {
          var _stack2 = [];
          try {
            const _cleanup = __using(_stack2, closing.join("cleanup"));
            await this.onClose();
          } catch (_2) {
  ...
  async onCreate() {
    ...
    if (duplicates.length) {
      var _stack = [];
      try {
        const _deduplicating = __using(_stack, this.#lifetime.join("deduplicating"));
  ...
  get isPrivacyMasked() {
    ...
    if (this.duration >= import_types.STANDARD_COMMISSIONING_TIMEOUT) {
      return true;
    }
    return false;
  }
   ...
   */
  get duration() {
    return (0, import_general.Timespan)(this.#startedAt, import_general.Time.nowMs).duration;
  }
  /**
...
class ActivityContext extends import_general.CancelablePromise {
  ...
  sleep(name, duration) {
    ...
    const sleep = import_general.Time.sleep(name, duration).finally(() => {
      if (this.#sleep === sleep) {
        this.#sleep = void 0;
      }
    });
