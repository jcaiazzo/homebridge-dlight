...
function Write(optionsOrData, ...data) {
  ...
  const result = {
    ...
    [import_general.Diagnostic.value]: () => {
      const items = data.flatMap((entry) => {
        const { version, value, endpoint, cluster, attributes } = entry;
        const valueString = import_general.Diagnostic.json(value);
        const list = Array.isArray(attributes) ? attributes : [attributes];
        const versionString = version !== void 0 ? `(version=${version})` : "";
        return list.map(
          (attribute) => import_general.Diagnostic.squash(
            import_general.Diagnostic.strong((0, import_Specifier.resolvePathForSpecifier)({ endpoint, cluster, attribute })),
            ` = ${valueString}${versionString}`
          )
        );
      });
      if (chunked) {
        return [import_general.Diagnostic.asFlags({ chunked: true }), import_general.Diagnostic.list(items)];
      }
      return import_general.Diagnostic.list(items);
    }
  };
