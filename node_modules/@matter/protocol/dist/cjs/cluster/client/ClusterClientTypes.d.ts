...
import { AttributeId, ClusterId, ClusterType, ClusterTyping, CommandId, EndpointNumber, EventId, EventNumber, TlvEventFilter, TypeFromSchema } from "@matter/types";
...
type ClientCommand<F> = F extends (...args: infer A) => infer Ret ? A extends [infer R, ...unknown[]] ? (request: R, options?: ClientCommandOptions) => Promise<Awaited<Ret>> : (options?: ClientCommandOptions) => Promise<Awaited<Ret>> : (options?: ClientCommandOptions) => Promise<void>;
type ClientCommands<C> = {
    [K in keyof C]: ClientCommand<C[K]>;
};
type ClientAttributeGetters<A> = {
    [K in RequiredKeys<A> & string as `get${Capitalize<K>}Attribute`]: (requestFromRemote?: boolean, isFabricFiltered?: boolean) => Promise<A[K]>;
} & {
    [K in OptionalKeys<A> as `get${Capitalize<K>}Attribute`]: (requestFromRemote?: boolean, isFabricFiltered?: boolean) => Promise<A[K] | undefined>;
};
...
type ClientAttributeSubscribers<A> = {
    [K in keyof A & string as `subscribe${Capitalize<K>}Attribute`]: (listener: (value: A[K]) => void, minIntervalS: number, maxIntervalS: number, knownDataVersion?: number, isFabricFiltered?: boolean) => Promise<void>;
};
...
type ClientEventGetters<E> = {
    [K in keyof E & string as `get${Capitalize<K>}Event`]: (minimumEventNumber?: number | bigint, isFabricFiltered?: boolean) => Promise<DecodedEventData<E[K]>[]>;
};
type ClientEventSubscribers<E> = {
    [K in keyof E & string as `subscribe${Capitalize<K>}Event`]: (listener: (value: DecodedEventData<E[K]>) => void, minIntervalS: number, maxIntervalS: number, isUrgent?: boolean, minimumEventNumber?: number | bigint, isFabricFiltered?: boolean) => Promise<void>;
};
type ClientEventListeners<E> = {
    [K in keyof E & string as `add${Capitalize<K>}EventListener`]: (listener: (value: DecodedEventData<E[K]>) => void) => void;
} & {
    [K in keyof E & string as `remove${Capitalize<K>}EventListener`]: (listener: (value: DecodedEventData<E[K]>) => void) => void;
};
...
} & ClientAttributeGetters<N["Attributes"]> & ClientLocalAttributeGetters<N["Attributes"]> & ClientAttributeSetters<N["Attributes"]> & ClientAttributeSubscribers<N["Attributes"]> & ClientAttributeListeners<N["Attributes"]> & ClientCommands<N["Commands"]> & ClientEventGetters<N["Events"]> & ClientEventSubscribers<N["Events"]> & ClientEventListeners<N["Events"]>;
