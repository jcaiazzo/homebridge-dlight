 ...
 */
const logger = import_Logger.Logger.get("Semaphore");
class Semaphore {
  #scope;
  #delay;
  #timeout;
  ...
  #closed = false;
  constructor(scope, concurrency = 1, delay = import_TimeUnit.Instant, timeout) {
    this.#scope = scope;
    this.#concurrency = concurrency;
    this.#delay = delay;
    this.#timeout = timeout;
    this.#delayTimer = import_Time.Time.getTimer("Queue delay", this.#delay, () => this.#processNextInQueue());
  }
  /**
  ...
  async obtainSlot(abort) {
    ...
    try {
      ...
      const combinedAbort = __using(_stack, new import_Abort.Abort({ abort: abort ? [abort, this.#abort] : [this.#abort] }));
  ...
  #grantSlot() {
    ...
    if (this.#timeout !== void 0) {
      timeoutTimer = import_Time.Time.getTimer(`Slot ${this.#scope} timeout`, this.#timeout, () => {
        if (!released) {
          logger.debug(
            `[${this.#scope}] Slot timed out after ${import_Duration.Duration.format(this.#timeout)}, auto-releasing`
          );
          release();
        }
      }).start();
