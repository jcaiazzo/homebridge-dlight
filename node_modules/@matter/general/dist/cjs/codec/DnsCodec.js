...
const TxtRecord = (name, entries, ttl = DEFAULT_MDNS_TTL, flushCache = false) => ({
  name,
  value: entries.map((e) => typeof e === "string" ? import_Bytes.Bytes.fromString(e) : import_Bytes.Bytes.of(e)),
  ttl,
  recordType: 16 /* TXT */,
  recordClass: 1 /* IN */,
  flushCache
...
class DnsCodec {
  ...
  static encodeTxtRecord(entries) {
    ...
    entries.forEach((entry) => {
      let bytes = typeof entry === "string" ? import_Bytes.Bytes.fromString(entry) : import_Bytes.Bytes.of(entry);
      if (bytes.byteLength > 255) {
        logger.warn(
          `TXT record entry length ${bytes.byteLength} exceeds the RFC 6763 \xA76.1 limit of 255 bytes; truncating.`
        );
        bytes = import_Bytes.Bytes.of(bytes).subarray(0, 255);
      }
